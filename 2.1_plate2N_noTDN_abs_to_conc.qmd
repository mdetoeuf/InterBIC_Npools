...
In this case, all "untrusted wells" (A1 in 4 panels of the last plot) are to be removed, as the values in A1 are clearly out of the curve (not rare: failure to eject liquid with the automated pipette). Although for NH4_PP1, the wrong A-well is in column 12, so in this case due to another issue. Nevertheless, the misalignment with the curve is obvious enough to be considered for removal.
