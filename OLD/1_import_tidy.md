...
- Incorporate the case of import from a Skanit-stuctured csv (function
  already exists: skanit_to_csv)
...
1.  Absorbance data stricto sensu = quantitative values for absorbance,
    ...
    - or as a skanit-structured csv[^1] (see
      <a href="#fig-skanit-csv" class="quarto-xref">Figure 3</a>)
...
2.  Plate map data, i.e., the map of well attribution to samples,
    ...
    <a href="#fig-csv-map-screenshot" class="quarto-xref">Figure 4</a>)
...
3.  Plate metadata, i.e., additional information that gives information
    ...
    (see
    <a href="#fig-metadata-screenshot" class="quarto-xref">Figure 5</a>)
...
<div id="fig-skanit-csv">

<img src="images/clipboard-1891204022.png" width="477" />

Figure 3: Typical structure of a csv as part of the output generated by
Skanit (here, the csv was exported from the first sheet of an xlsx file
provided by Skanit). If the structure of your file is slightly
different, adaptations to the import function might have to be made. If
so, please reach out!
...
Figure 4: One example of plate map data in the same .csv format as the
...
Figure 5: Example of plate metadata. There could be more or less
...
source("functions/skanit_to_csv.R")
```
...
Skanit_abs <- import_data_plate(
  format_abs = "skanit", 
  skanit_csv = "raw_data/Skanit_example/MR_R1_t0.csv",
  skanit_abs_or_map = "maps")
```
...
>
> The previous chunk (absorbance data) and the next one (plate maps
> data) demonstrate the usage of the Skanit format version of the
> function `import_data_plate()`, but the rest of this pipeline does not
> require the Skanit data, so it is no longer used passed this import
> step. However, the output format should, in theory, be strictly
> identical to other format options, so that the object `Skanit_abs`
> should, in theory, be interchangeable with `TDN_abs` or `Nmin_abs`.
> Please reach out if you encounter any difficulty.
...
[^1]: Skanit is the software that collects absorbance data on your
    computer if you connect it to the plate-reader that is available at
    the Agroecology Lab
